...
if($result = mysqli_query($link, $sql)){
    if(mysqli_num_rows($result)>0){
        while($row = mysqli_fetch_assoc($result)){
            ...
            echo "
        <div class='note'>
            <div class='mt-2 noteheader w-75 float-left' id='$note_id'>
                <div class='text'>$note</div>
                <div class='timetext'>$time</div>    
            </div>
            <div class=' mt-2 w-25 float-right delete'>
                <button class='btn-lg ml-1 btn-danger' style='width:100%'>delete</button>
            </div>
        </div>";
        }
    }else{
